...
$(document).ready(function() {
	...
	$('#ctrl-do_you_want_to_apply_with_full_potential_plan').on('change', function(){ 
	var ctrlVal = $(this).val();
    var $field = $("#ctrl-upload_the_full_potential_plan");
    var $formGroup = $field.parents(".form-group");
    if(ctrlVal == "NO"){
        $field.removeAttr("required"); // Remove required validation
        $formGroup.hide(); // Hide the field
    } else {
        $field.attr("required", true); // Add required validation
        $field.val(""); // Reset the field
        $formGroup.show(); // Show the field
    }
});
$('#ctrl-is_redevelopment').on('change', function(){ 
    var ctrlVal = $(this).val();
    var $field = $("#ctrl-upload_society_consent_or_recommendation_letter");
    var $formGroup = $field.parents(".form-group");
    if(ctrlVal == "NO"){
        $field.removeAttr("required"); // Remove required validation
        $formGroup.hide(); // Hide the field
...
var floorTypeName = $('#ctrl-typical_floor_plan').val(); // Get tender number from URL or field
if (floorTypeName) { // Only call API if tender number exists
    fetchFloorInfo(floorTypeName);
}
// Trigger when tender number changes manually
$('#ctrl-typical_floor_plan').on('change', function() { 
    var val = $(this).val();
    fetchFloorInfo(val);
});
function fetchFloorInfo(val) {
    $.getJSON(siteAddr + "api/getfloorinfo/" + val, function(data) {
        $('#ctrl-refuge_area').val(data.refuge_area);
        $('#ctrl-floor_type').val(data.floor_type);
        $('#ctrl-foor_name_id').val(data.foor_name_id);
        $('#ctrl-floor_wise_p_line_area_in_sqr_mtr').val(data.floor_wise_p_line_area_in_sqr_mtr);
        $('#ctrl-hight_in_mtr_from_ground').val(data.hight_in_mtr_from_ground);
        $('#ctrl-hight_of_refuge_area_from_ground_in_mtr').val(data.hight_of_refuge_area_from_ground_in_mtr);
        $('#ctrl-provided_refuge_area_in_sqr_mtr').val(data.provided_refuge_area_in_sqr_mtr);
    });
}
$('#ctrl-refuge_area').on('change', function(){ 
	//do something like 
	//$(this).hide(); 
	//$(#anotherfieldid).show();
	var cv = $(this).val();
	if(cv=="NO"){
	    $("#ctrl-hight_of_refuge_area_from_ground_in_mtr").parents(".form-group").hide();
	    $("#ctrl-provided_refuge_area_in_sqr_mtr").parents(".form-group").hide();
		     $("#ctrl-hight_of_refuge_area_from_ground_in_mtr").removeAttr('required'  );
	    $("#ctrl-provided_refuge_area_in_sqr_mtr").removeAttr('required');
	}else{
	     $("#ctrl-hight_of_refuge_area_from_ground_in_mtr").parents(".form-group").show();
	     $("#ctrl-hight_of_refuge_area_from_ground_in_mtr").prop('required',true);
	    $("#ctrl-provided_refuge_area_in_sqr_mtr").parents(".form-group").show();
	    $("#ctrl-provided_refuge_area_in_sqr_mtr").prop('required',true);
	}
	//$(#anotherfieldname).val(ctrlVal)
});
$('#ctrl-property_number').on('change', function(){ 
    var propNo = $(this).val().trim();
    if (propNo !== '') {
        var apiUrl = siteAddr + "getpropertydata.php?p=" + encodeURIComponent(propNo);
        $.getJSON(apiUrl, function(response) {
            if (response && response.GetPropDetailsClassList && response.GetPropDetailsClassList.length > 0) {
                var propDetails = response.GetPropDetailsClassList[0];
                $('#ctrl-name_of_property_owner').val(propDetails.Name);
                $('#ctrl-pending_due_amount').val(propDetails.Payable_Amount);
                $('#ctrl-area_in_sq_ft').val(propDetails.Area);
            } else {
                // clear fields if no result
                $('#ctrl-name_of_property_owner').val('');
                $('#ctrl-pending_due_amount').val('');
                $('#ctrl-area_in_sq_ft').val('');
                alert("No property data found for: " + propNo);
            }
        }).fail(function() {
...
$('#ctrl-establishment_type_id').on('change', function(){ 
	//do something like 
	//$(this).hide(); 
	//$(#anotherfieldid).show();
	var xv = $(this).val();
	if(xv=="1"){
	    $("#ctrl-subject").val("हॉटेल अग्निशमन विषयक दाखला मिळणेबाबत");
	}
	if(xv=="2"){
	    $("#ctrl-subject").val("औद्योगिक कारखाने अग्निशमन विषयक दाखला मिळणेबाबत");
	}
	if(xv=="3"){
	    $("#ctrl-subject").val("खाजगी रुग्णालय अग्निशमन विषयक दाखला मिळणेबाबत");
	}
	if(xv=="4"){
	    $("#ctrl-subject").val("सरकारी रुग्णालयअग्निशमन विषयक दाखला मिळणेबाबत");
	}
	if(xv=="5"){
	    $("#ctrl-subject").val("शाळा/महाविद्यालय/कोचिंग क्लासेस अग्निशमन विषयक दाखला मिळणेबाबत");
	}
	if(xv=="6"){
	    $("#ctrl-subject").val("मॉल/सिनेमा अग्निशमन विषयक दाखला मिळणेबाबत");
	}
	if(xv=="7"){
	    $("#ctrl-subject").val("इतर अग्निशमन विषयक दाखला मिळणेबाबत");
	} 
	//$(#anotherfieldname).val(ctrlVal)
});
$('#ctrl-is_structural_audit_report_avilable_id').on('change', function(){ 
	//do something like 
	//$(this).hide(); 
	//$(#anotherfieldid).show();
	var ctrlVal = $(this).val();
	if(ctrlVal=="1"){
	    $("#ctrl-structural_audit_report_date").parents(".form-group").show();
	    $("#ctrl-structural_audit_report_date").prop("required",true);
	}else{
	    $("#ctrl-structural_audit_report_date").parents(".form-group").hide();
	    $("#ctrl-structural_audit_report_date").removeAttr("required");
	}
	//$(#anotherfieldname).val(ctrlVal)
});
$('#ctrl-is_electrical_audit_report_available_id').on('change', function(){ 
	//do something like 
	//$(this).hide(); 
	//$(#anotherfieldid).show(); 
	var ctrlVal = $(this).val();
	if(ctrlVal=="1"){
	    $("#ctrl-electrical_audit_report_date").parents(".form-group").show();
	    $("#ctrl-electrical_audit_report_date").prop("required",true);
	}else{
	    $("#ctrl-electrical_audit_report_date").parents(".form-group").hide();
	    $("#ctrl-electrical_audit_report_date").removeAttr("required");
	}
	//$(#anotherfieldname).val(ctrlVal)
});
$('#ctrl-is_revised_provisional_taken').on('change', function(){ 
	var ctrlVal = $(this).val();
	if(ctrlVal=="YES"){
	    $("#ctrl-provisional_noc_number").parents(".form-group").show();
	    $("#ctrl-provisional_noc_number").prop('required',true);
	}else{
	    $("#ctrl-provisional_noc_number").parents(".form-group").hide();
	    $("#ctrl-provisional_noc_number").removeAttr('required');
	}
});
$('#ctrl-is_revised_noc_taken').on('change', function(){ 
	//do something like 
	//$(this).hide(); 
	//$(#anotherfieldid).show();
	var ctrlVal = $(this).val();
	if(ctrlVal=="YES"){
	    $("#ctrl-revised_provisional_noc_number").parents(".form-group").show();
	    $("#ctrl-revised_provisional_noc_number").prop('required',true);
	}else{
	    $("#ctrl-revised_provisional_noc_number").parents(".form-group").hide();
	    $("#ctrl-revised_provisional_noc_number").removeAttr('required');
	}
	//$(#anotherfieldname).val(ctrlVal)
});
$('#ctrl-revised_provisional_noc_number').on('change', function(){ 
});
$('#ctrl-change_of_usage').on('input', function() {
    var selectedVal = this.value;
    var $uploadField = $('#ctrl-upload_change_of_usage_certificate');
    var $formGroup = $uploadField.closest('.form-group');
    if (selectedVal === "NO") {
        $uploadField.val('');                   // clear file input
        $formGroup.hide();                      // hide the field
        $uploadField.prop('disabled', true);    // disable input
    } else {
        $formGroup.show();                      // show the field
        $uploadField.prop('disabled', false);   // enable input
        $uploadField.attr('required', true);    // make required
    }
});
$('#ctrl-provisional_noc_number').on('change', function(){ 
});
$('#ctrl-payment_type').on('change', function(){ 
	//do something like 
	//$(this).hide(); 
	//$(#anotherfieldid).show();
	var ctrlVal = $(this).val();
	if(ctrlVal=="CASH"){
	   ...
	   // $("#ctrl-upload_dd_or_cheque").parents(".form-group").hide();
	    $("#ctrl-payment_bankname").parents(".form-group").hide();
	    $("#ctrl-payment_chq_no").parents(".form-group").removeAttr('required',true);
	    $("#ctrl-payment_chq_date").parents(".form-group").removeAttr('required',true);
	   // $("#ctrl-upload_dd_or_cheque").parents(".form-group").removeAttr('required',true);
	    $("#ctrl-payment_bankname").parents(".form-group").removeAttr('required',true);
	}
	if(ctrlVal=="CHEQUE" || ctrlVal=="DD"){
	    $("#ctrl-payment_chq_no").parents(".form-group").show();
	    $("#ctrl-payment_chq_date").parents(".form-group").show();
	   // $("#ctrl-upload_dd_or_cheque").parents(".form-group").show();
	    $("#ctrl-payment_bankname").parents(".form-group").hide();
	    $("#ctrl-payment_chq_no").parents(".form-group").prop('required',true);
	    $("#ctrl-payment_chq_date").parents(".form-group").prop('required',true);
	   // $("#ctrl-upload_dd_or_cheque").parents(".form-group").prop('required',true);
	    $("#ctrl-payment_bankname").parents(".form-group").removeAttr('required',true);
	}
	if(ctrlVal=="BANK"){
	    $("#ctrl-payment_chq_no").parents(".form-group").hide();
	    $("#ctrl-payment_chq_date").parents(".form-group").hide();
	   // $("#ctrl-upload_dd_or_cheque").parents(".form-group").hide();
	    $("#ctrl-payment_bankname").parents(".form-group").show();
	    $("#ctrl-payment_chq_no").parents(".form-group").removeAttr('required',true);
	    $("#ctrl-payment_chq_date").parents(".form-group").removeAttr('required',true);
	   // $("#ctrl-upload_dd_or_cheque").parents(".form-group").removeAttr('required',true);
	    $("#ctrl-payment_bankname").parents(".form-group").prop('required',true);
	}
	//$(#anotherfieldname).val(ctrlVal)
});
$('#ctrl-do_you_have_wing').on('change', function(){ 
	//do something like 
	//$(this).hide(); 
	//$(#anotherfieldid).show();
	var cv = $(this).val();
	if(cv=="NO"){
	    $("label[for^='number_of_wing_in_building']").html("Enter Number of Floors <span class='text-danger'>*</span>");
	    $("#ctrl-number_of_wing_in_building").prop("placeholder","Enter Number of Floors");
	}else{
	    $("label[for^='number_of_wing_in_building']").html("Number Of Wing In Building <span class='text-danger'>*</span>");
	    $("#ctrl-number_of_wing_in_building").prop("placeholder","Number Of Wing In Building");
	}
	//$(#anotherfieldname).val(ctrlVal)
});
});
...
*/
$(document).ready(function() {
	$("#ctrl-type_of_application_id").on("change",function(){
	    if($(this).val()=="1"){
	        $("#ctrl-old_noc_number").parents(".form-group").hide();
	        $("#ctrl-old_noc_date").parents(".form-group").hide();
	    }else{
	        $("#ctrl-old_noc_number").parents(".form-group").show();
	        $("#ctrl-old_noc_date").parents(".form-group").show();
	    }
	});
});
$('#ctrl-mobile_no').on('change', function(){ 
	$lent = $(this).val().length;
      if ($lent < 9 || $lent > 10 )
      {
          $(this).val("");     /*  to clear the record     */
           $(this).focus();
