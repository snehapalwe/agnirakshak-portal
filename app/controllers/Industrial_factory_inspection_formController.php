<?php 
/**
 * Industrial_factory_inspection_form Page Controller
 * @category  Controller
 */
class Industrial_factory_inspection_formController extends SecureController{
	function __construct(){
		parent::__construct();
		$this->tablename = "industrial_factory_inspection_form";
	}
	/**
     * List page records
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($request->search)){
			...
			$search_condition = "(
				industrial_factory_inspection_form.id LIKE ? OR 
				industrial_factory_inspection_form.business_name LIKE ? OR 
				industrial_factory_inspection_form.business_address LIKE ? OR 
				industrial_factory_inspection_form.mobile_no LIKE ? OR 
				industrial_factory_inspection_form.owners_name LIKE ? OR 
				industrial_factory_inspection_form.residential_address LIKE ? OR 
				industrial_factory_inspection_form.owners_mobile_number LIKE ? OR 
				industrial_factory_inspection_form.type_of_application_id LIKE ? OR 
				industrial_factory_inspection_form.type_of_application_value LIKE ? OR 
				industrial_factory_inspection_form.old_noc_number LIKE ? OR 
				industrial_factory_inspection_form.old_noc_date LIKE ? OR 
				industrial_factory_inspection_form.building_type LIKE ? OR 
				industrial_factory_inspection_form.building_height LIKE ? OR 
				industrial_factory_inspection_form.building_floor LIKE ? OR 
				industrial_factory_inspection_form.no_of_staircase_in_building_and_width LIKE ? OR 
				industrial_factory_inspection_form.no_of_commercial_spaces LIKE ? OR 
				industrial_factory_inspection_form.structural_audit_report_year_of_construction LIKE ? OR 
				industrial_factory_inspection_form.structural_audit_report_date LIKE ? OR 
				industrial_factory_inspection_form.structural_audit_agency_name LIKE ? OR 
				industrial_factory_inspection_form.structural_audit_agency_address LIKE ? OR 
				industrial_factory_inspection_form.structural_audit_agency_mobile_no LIKE ? OR 
				industrial_factory_inspection_form.permissions_obtained_for_the_business LIKE ? OR 
				industrial_factory_inspection_form.female_employees_count LIKE ? OR 
				industrial_factory_inspection_form.male_employees_count LIKE ? OR 
				industrial_factory_inspection_form.total_employees_count LIKE ? OR 
				industrial_factory_inspection_form.working_hours_at_business_premises LIKE ? OR 
				industrial_factory_inspection_form.natural_ventilation_total_windows LIKE ? OR 
				industrial_factory_inspection_form.natural_ventilation_total_doors LIKE ? OR 
				industrial_factory_inspection_form.area_of_business_premises LIKE ? OR 
				industrial_factory_inspection_form.entrance_route_premises LIKE ? OR 
				industrial_factory_inspection_form.exit_route_premises LIKE ? OR 
				industrial_factory_inspection_form.record_room LIKE ? OR 
				industrial_factory_inspection_form.water_storage_capacity_terrace LIKE ? OR 
				industrial_factory_inspection_form.water_storage_capacity_groundfloor LIKE ? OR 
				industrial_factory_inspection_form.no_lifts_in_the_building LIKE ? OR 
				industrial_factory_inspection_form.capacity_lifts_in_the_building LIKE ? OR 
				industrial_factory_inspection_form.electrical_connection_capacity LIKE ? OR 
				industrial_factory_inspection_form.electrical_audit_report_date LIKE ? OR 
				industrial_factory_inspection_form.electrical_audit_agency_name LIKE ? OR 
				industrial_factory_inspection_form.electrical_audit_agency_address LIKE ? OR 
				industrial_factory_inspection_form.electrical_audit_agency_mobile_no LIKE ? OR 
				industrial_factory_inspection_form.generator_system LIKE ? OR 
				industrial_factory_inspection_form.raw_material_name LIKE ? OR 
				industrial_factory_inspection_form.raw_material_storage_capacity LIKE ? OR 
				industrial_factory_inspection_form.no_of_lpg_gas_cylinders LIKE ? OR 
				industrial_factory_inspection_form.name_of_final_product LIKE ? OR 
				industrial_factory_inspection_form.storage_capacity_of_final_product LIKE ? OR 
				industrial_factory_inspection_form.fire_extinguishing_permanent LIKE ? OR 
				industrial_factory_inspection_form.fire_extinguishing_temporary LIKE ? OR 
				industrial_factory_inspection_form.extinguishing_license_agency_name LIKE ? OR 
				industrial_factory_inspection_form.extinguishing_license_agency_lno LIKE ? OR 
				industrial_factory_inspection_form.extinguishing_license_agency_lvalidity LIKE ? OR 
				industrial_factory_inspection_form.extinguishing_licen_agency_cert_no LIKE ? OR 
				industrial_factory_inspection_form.user_id LIKE ? OR 
				industrial_factory_inspection_form.date LIKE ? OR 
				industrial_factory_inspection_form.status LIKE ? OR 
				industrial_factory_inspection_form.paid LIKE ? OR 
				industrial_factory_inspection_form.int_no LIKE ? OR 
				industrial_factory_inspection_form.yr LIKE ? OR 
				industrial_factory_inspection_form.zone LIKE ? OR 
				industrial_factory_inspection_form.payment_done LIKE ? OR 
				industrial_factory_inspection_form.certificate_file LIKE ? OR 
				industrial_factory_inspection_form.timestamp LIKE ? OR 
				industrial_factory_inspection_form.recid LIKE ?
			)";
			$search_params = array(
			...
			$this->view->search_template = "industrial_factory_inspection_form/search.php";
		}
		if(!empty($request->orderby)){
			...
			$db->orderBy("industrial_factory_inspection_form.id", ORDER_TYPE);
		}
		if($fieldname){
...
$record['extinguishing_license_agency_lvalidity'] = human_date($record['extinguishing_license_agency_lvalidity']);
		...
		$page_title = $this->view->page_title = "Industrial Factory Inspection Form";
		$this->render_view("industrial_factory_inspection_form/list.php", $data); //render the full page
	}
	/**
     * View record detail 
	...
	function view($rec_id = null, $value = null){
		...
		if($value){
			...
			$db->where("industrial_factory_inspection_form.id", $rec_id);; //select record based on primary key
		}
...
$record['electrical_audit_report_date'] = human_date($record['electrical_audit_report_date']);
$record['extinguishing_license_agency_lvalidity'] = human_date($record['extinguishing_license_agency_lvalidity']);
			$page_title = $this->view->page_title = "View  Industrial Factory Inspection Form";
		...
		return $this->render_view("industrial_factory_inspection_form/view.php", $record);
	}
	/**
	...
	function add($formdata = null){
		if($formdata){
			...
			$this->rules_array = array(
				...
				'owners_mobile_number' => 'required|numeric|max_numeric,9999999999|min_numeric,0000000000',
...
$db->update("fire_noc_establishment",["inspection_done"=>$rec_id]);
		...
		$page_title = $this->view->page_title = "Add New Industrial Factory Inspection Form";
		$this->render_view("industrial_factory_inspection_form/add.php");
	}
	/**
...
$modeldata['status'] = "0";
			if($this->validated()){
				$db->where("industrial_factory_inspection_form.id", $rec_id);;
				$bool = $db->update($tablename, $modeldata);
				$numRows = $db->getRowCount(); //number of affected rows. 0 = no record field updated
				if($bool && $numRows){
					$this->set_flash_msg("Record updated successfully", "success");
					return $this->redirect("industrial_factory_inspection_form");
				}
				else{
					if($db->getLastError()){
						$this->set_page_error();
		...
		$page_title = $this->view->page_title = "Edit  Industrial Factory Inspection Form";
		if(!$data){
			$this->set_page_error();
		}
		return $this->render_view("industrial_factory_inspection_form/edit.php", $data);
	}
	/**
	...
	function delete($rec_id = null){
		...
		$db->where("industrial_factory_inspection_form.id", $arr_rec_id, "in");
