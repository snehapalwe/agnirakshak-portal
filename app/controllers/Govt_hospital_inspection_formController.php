<?php 
/**
 * Govt_hospital_inspection_form Page Controller
 * @category  Controller
 */
class Govt_hospital_inspection_formController extends SecureController{
	function __construct(){
		parent::__construct();
		$this->tablename = "govt_hospital_inspection_form";
	}
	/**
     * List page records
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($request->search)){
			...
			$search_condition = "(
				govt_hospital_inspection_form.id LIKE ? OR 
				govt_hospital_inspection_form.recid LIKE ? OR 
				govt_hospital_inspection_form.hospital_or_health_center_name LIKE ? OR 
				govt_hospital_inspection_form.hospital_or_health_center_address LIKE ? OR 
				govt_hospital_inspection_form.doctor_name LIKE ? OR 
				govt_hospital_inspection_form.doctor_mobile_no LIKE ? OR 
				govt_hospital_inspection_form.building_height LIKE ? OR 
				govt_hospital_inspection_form.lifts_company_name LIKE ? OR 
				govt_hospital_inspection_form.no_lifts_in_building LIKE ? OR 
				govt_hospital_inspection_form.capacity_of_lifts LIKE ? OR 
				govt_hospital_inspection_form.elevators_amc_done_by_org_name LIKE ? OR 
				govt_hospital_inspection_form.elevators_amc_period LIKE ? OR 
				govt_hospital_inspection_form.number_of_staircase_in_bulding LIKE ? OR 
				govt_hospital_inspection_form.width_of_staircase_in_bulding LIKE ? OR 
				govt_hospital_inspection_form.entrance_routes_count LIKE ? OR 
				govt_hospital_inspection_form.exit_routes_count LIKE ? OR 
				govt_hospital_inspection_form.is_record_room_available LIKE ? OR 
				govt_hospital_inspection_form.hospital_area LIKE ? OR 
				govt_hospital_inspection_form.doctors_count LIKE ? OR 
				govt_hospital_inspection_form.nurses_count LIKE ? OR 
				govt_hospital_inspection_form.ward_boy_count LIKE ? OR 
				govt_hospital_inspection_form.aunts_count LIKE ? OR 
				govt_hospital_inspection_form.clerk_count LIKE ? OR 
				govt_hospital_inspection_form.other_employees_count LIKE ? OR 
				govt_hospital_inspection_form.floor_number_of_hospital_id LIKE ? OR 
				govt_hospital_inspection_form.floor_number_of_hospital_value LIKE ? OR 
				govt_hospital_inspection_form.is_ot_dept LIKE ? OR 
				govt_hospital_inspection_form.is_xray_dept LIKE ? OR 
				govt_hospital_inspection_form.is_opd_dept LIKE ? OR 
				govt_hospital_inspection_form.is_emergency_opd_dept LIKE ? OR 
				govt_hospital_inspection_form.is_patholoty_dept LIKE ? OR 
				govt_hospital_inspection_form.is_post_nutal_care_dept LIKE ? OR 
				govt_hospital_inspection_form.is_icu_dept LIKE ? OR 
				govt_hospital_inspection_form.is_gw_men_dept LIKE ? OR 
				govt_hospital_inspection_form.is_gw_women_dept LIKE ? OR 
				govt_hospital_inspection_form.is_special_ward_dept LIKE ? OR 
				govt_hospital_inspection_form.is_ante_nutal_care_dept LIKE ? OR 
				govt_hospital_inspection_form.is_nicu_dept LIKE ? OR 
				govt_hospital_inspection_form.directional_board LIKE ? OR 
				govt_hospital_inspection_form.total_no_beds_hospital LIKE ? OR 
				govt_hospital_inspection_form.generator_system_capacity LIKE ? OR 
				govt_hospital_inspection_form.generator_system_amc_org_name LIKE ? OR 
				govt_hospital_inspection_form.generator_system_amc_period LIKE ? OR 
				govt_hospital_inspection_form.electrical_audit_report_org_name LIKE ? OR 
				govt_hospital_inspection_form.electrical_audit_report_date LIKE ? OR 
				govt_hospital_inspection_form.emergency_power_system LIKE ? OR 
				govt_hospital_inspection_form.info_about_fire_prevention_measures LIKE ? OR 
				govt_hospital_inspection_form.mock_drill_date LIKE ? OR 
				govt_hospital_inspection_form.no_emp_present_for_mock_drill LIKE ? OR 
				govt_hospital_inspection_form.upload_photo_of_employee_present_for_mock_drill LIKE ? OR 
				govt_hospital_inspection_form.upload_fire_marshal_names_with_mobile_no LIKE ? OR 
				govt_hospital_inspection_form.water_storage_capacity_terrace LIKE ? OR 
				govt_hospital_inspection_form.water_storage_capacity_groundfloor LIKE ? OR 
				govt_hospital_inspection_form.fire_noc_details LIKE ? OR 
				govt_hospital_inspection_form.fire_noc_date LIKE ? OR 
				govt_hospital_inspection_form.other_info_about_hospital LIKE ? OR 
				govt_hospital_inspection_form.oxygen_system LIKE ? OR 
				govt_hospital_inspection_form.maintenance_of_emergency_routes LIKE ? OR 
				govt_hospital_inspection_form.remark LIKE ? OR 
				govt_hospital_inspection_form.user_id LIKE ? OR 
				govt_hospital_inspection_form.date LIKE ? OR 
				govt_hospital_inspection_form.status LIKE ? OR 
				govt_hospital_inspection_form.paid LIKE ? OR 
				govt_hospital_inspection_form.int_no LIKE ? OR 
				govt_hospital_inspection_form.yr LIKE ? OR 
				govt_hospital_inspection_form.zone LIKE ? OR 
				govt_hospital_inspection_form.payment_done LIKE ? OR 
				govt_hospital_inspection_form.certificate_file LIKE ?
			)";
			$search_params = array(
			...
			$this->view->search_template = "govt_hospital_inspection_form/search.php";
		}
		if(!empty($request->orderby)){
			...
			$db->orderBy("govt_hospital_inspection_form.id", ORDER_TYPE);
		}
		if($fieldname){
...
$record['fire_noc_date'] = human_date($record['fire_noc_date']);
		...
		$page_title = $this->view->page_title = "Govt Hospital Inspection Form";
		$this->view->report_filename = date('Y-m-d') . '-' . $page_title;
		$this->view->report_title = $page_title;
		$this->view->report_layout = "report_layout.php";
		$this->view->report_paper_size = "A4";
		$this->view->report_orientation = "portrait";
		$this->render_view("govt_hospital_inspection_form/list.php", $data); //render the full page
	}
	/**
     * View record detail 
	...
	function view($rec_id = null, $value = null){
		...
		if($value){
			...
			$db->where("govt_hospital_inspection_form.id", $rec_id);; //select record based on primary key
		}
...
$record['mock_drill_date'] = human_datetime($record['mock_drill_date']);
$record['fire_noc_date'] = human_date($record['fire_noc_date']);
			$page_title = $this->view->page_title = "View  Govt Hospital Inspection Form";
		$this->view->report_filename = date('Y-m-d') . '-' . $page_title;
		$this->view->report_title = $page_title;
		$this->view->report_layout = "report_layout.php";
		$this->view->report_paper_size = "A4";
		...
		return $this->render_view("govt_hospital_inspection_form/view.php", $record);
	}
	/**
...
$db->update("fire_noc_establishment",["inspection_done"=>$rec_id]);
		...
		$page_title = $this->view->page_title = "Add New Govt Hospital Inspection Form";
		$this->render_view("govt_hospital_inspection_form/add.php");
	}
	/**
...
$modeldata['status'] = "0";
			if($this->validated()){
				$db->where("govt_hospital_inspection_form.id", $rec_id);;
				$bool = $db->update($tablename, $modeldata);
				$numRows = $db->getRowCount(); //number of affected rows. 0 = no record field updated
				if($bool && $numRows){
					$this->set_flash_msg("Record updated successfully", "success");
					return $this->redirect("govt_hospital_inspection_form");
				}
				else{
					if($db->getLastError()){
						$this->set_page_error();
		...
		$page_title = $this->view->page_title = "Edit  Govt Hospital Inspection Form";
		if(!$data){
			$this->set_page_error();
		}
		return $this->render_view("govt_hospital_inspection_form/edit.php", $data);
	}
	/**
	...
	function delete($rec_id = null){
		...
		$db->where("govt_hospital_inspection_form.id", $arr_rec_id, "in");
