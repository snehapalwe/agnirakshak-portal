...
<?php 
    $jei = new User_infoController();
    $db  = $jei->GetModel();
    $db->where("user_id", get_active_user('id'));
    $auth = $db->getOne("authorization_sequence");
 
    $user_stage = $auth ? $auth['stage'] : null;
?>
<script>
$(document).ready(function () {
 
    var user_stage = <?php echo json_encode($user_stage); ?>;
 
    function applyStageVisibility() {
        // First show ALL options
        $('#ctrl-sendbackto option').prop('disabled', false).show();
 
        if (user_stage == "2") {
            // Stage 2: Show only Auth1 and Architect
            $('#ctrl-sendbackto option[value="2"]').prop('disabled', true).hide();   // hide Auth2
        }
 
        else if (user_stage == "3") {
            // Stage 3: Show Auth1, Auth2, Architect (no hiding)
            // So nothing to hide
        }
 
        // Stage 1 logic will apply ONLY when selecting "Revert"
        // So do nothing here for stage 1
    }
    ...
    // Stage 1: Apply revert logic only for stage = 1
    if (user_stage == "1") {
 
        $('#ctrl-action').on('change', function () {
 
            var actionVal = $(this).val().trim();
 
            if (actionVal === 'Revert') {
 
                // hide Auth1 & Auth2
                $('#ctrl-sendbackto option[value="1"]').prop('disabled', true).hide();
                $('#ctrl-sendbackto option[value="2"]').prop('disabled', true).hide();
 
                // reset selection
                if ($('#ctrl-sendbackto').val() === '1' || $('#ctrl-sendbackto').val() === '2') {
                    $('#ctrl-sendbackto').val('');
                }
 
            } else {
                // show again
                $('#ctrl-sendbackto option[value="1"]').prop('disabled', false).show();
                $('#ctrl-sendbackto option[value="2"]').prop('disabled', false).show();
            }
        });
