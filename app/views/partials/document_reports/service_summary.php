                            ...
                            <div class="row">
                                <div class="col">
                                    ...
                                    <select required=""  id="db_name" name="db_name" id="db_name"  placeholder="Select a value ..."    class="custom-select" > 
                                        ...
                                        <?php 
                                        $db_name_options = $comp_model -> document_reports_db_name_option_list();
                                        if(!empty($db_name_options)){
                                        foreach($db_name_options as $option){
                                        $value = (!empty($option['value']) ? $option['value'] : null);
                                        $label = (!empty($option['label']) ? $option['label'] : $value);
                                        $selected = $this->set_field_selected('db_name',$value, "");
                                        if($value=="accept_reject"||
                                        $value=="appointments"||
                                        $value=="attachement_of_other_form"||
                                        $value=="authorization_sequence"||
                                        $value=="certificate_uploads"||
                                        $value=="demand"||
                                        $value=="document_reports"||
                                        $value=="payments"||
                                        $value=="roles"||
                                ...
                                <div class="col "> 
                                    ...
                                    <script>
                                        ...
                                        location.href='?db_name='+val+'&detailed_ledger_date='+val2;
                                        }
                                        function compute_charts(){
                                        val=$("#db_name").val();
                                        val2=$("#detailed_ledger_date").val();
                                        $.getJSON( '<?php echo SITE_ADDR; ?>api/report_for_document/' + val  +'/' + val2)
                                        .done(function (json) {
                                        data=json.data;
                                        am5.ready(function() {
                                        // Create root element
